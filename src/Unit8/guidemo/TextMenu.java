...
public class TextMenu extends JMenu {
    ...
    public TextMenu(DrawPanel owner) {
        ...
        final JMenuItem lineH = new JMenuItem("Set Line Height");
        lineH.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                double currentHeight = panel.getTextItem().getLineHeightMultiplier();
                String s = JOptionPane.showInputDialog(panel, "By what amount would you want the line height " +
                        "to be multiplied by?", currentHeight);
                if (s != null && s.trim().length() > 0) {
                    try {
